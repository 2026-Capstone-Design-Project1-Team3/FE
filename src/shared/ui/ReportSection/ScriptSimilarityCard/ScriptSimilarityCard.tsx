...
interface ScriptSimilarityCardProps {
  percentage: number;
  subtitle: string;
  description: string;
}

export const ScriptSimilarityCard = ({
  percentage,
  subtitle,
  description,
}: ScriptSimilarityCardProps) => {
  const radius = 20;
  ...
  return (
    <div className="w-full rounded-2xl border border-border-default hover:bg-background-dark bg-background-light px-6 py-6 shadow-sm">
      <div className="mb-6 flex items-center gap-2.5">
        <FileText className="h-7 w-7 pb-1 text-primary-900" strokeWidth={2.5} />
        <h2 className="text-subtitle-01 text-text-primary">대본 유사도 분석</h2>
      </div>
...
export default ScriptSimilarityCard;
